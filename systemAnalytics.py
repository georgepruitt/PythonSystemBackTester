...
def calcSystemResults(systemMarketList):
    ...
    fileName5 = systemMarketList[0].systemName + "-Graph.txt"
#    target5 = open(fileName5,"w")
    target5 = open(fileName5,"w")
    lineOutPut = 
    target5.write
...
#            systemMarketList[i].tradesList[j].printTrade()
            if j < len(systemMarketList[i].tradesList)-1:
                if systemMarketList[i].tradesList[j].tradePrice != systemMarketList[i].tradesList[j+1].tradePrice:
                    row = str(systemMarketList[i].tradesList[j].tradeDate) +","+str(systemMarketList[i].tradesList[j].quant)+","
                    row2 = row + systemMarketList[i].tradesList[j].tradeOrder+","+str(systemMarketList[i].tradesList[j].entryOrExit)+","
                    row3 = row2 + str(systemMarketList[i].tradesList[j].tradePrice)+"\n"
                    target5.write(row3)
...
#            print(systemMarketList[i].equity.equityDate[j],' ',systemMarketList[i].equity.dailyEquityVal[j])
        ...
        for j in range(0,numOfMonths):
            ...
            if j == 0:
                ...
                if(int(monthList[j]/100) % 100) == 12:
                    yearEquity.append(cumulativeEquity[-1] - 0)
        ...
        for j in range(0,numOfMonths):
            ...
            if isDecember:
                if printToTerminal == 1: print('%8d %10.0f %10.0f %10.0f' % (monthList[j],monthEquity[j],cumulativeEquity[j],yearEquity[yearInc]))
